...
public class ArcadTableView extends ViewPart implements ISelectionChangedListener {
	...
	protected void hookContextMenu() {
		...
		if (hookTableMenu) {
			Menu menu = menuMgr.createContextMenu(viewer.getViewer().getControl());
			viewer.getViewer().getControl().setMenu(menu);
			// [ML] Le menu popup devrait pouvoir être surclassé par les
			// Descendants.
			getSite().registerContextMenu(menuMgr, viewer.getViewer());
		}
	}
	...
	public void setFocus() {
		try {
			if (viewer != null && viewer.getViewer() != null && viewer.getControl() != null)
