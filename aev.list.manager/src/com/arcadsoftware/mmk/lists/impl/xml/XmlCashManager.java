...
implements IListBrowseListener {
	...
	private int removeDuplicatItems(String orderQuery) throws ArcadException{
		...
		try {
			...
			while(rs.next()) {
				for (int i=0;i<list.getMetadatas().count();i++){
					//d‚calage de 2 du rs.getString() car commence … 1
				...
				if (!currentKey.equals(key)) {
					...
					//On ne comptabilise que les ‚l‚ments que l'on ne retient pas.
					count++;
				}
			}
			rs.close();
	...
	public int removeItems(StoreItem[] items) throws ArcadException {
		...
		//TODO [LM] mise en place d'une strat‚gie de sauvegarde du cache;
		flushRequest();
		return count;
	}
	...
	/* -----------------------------------------------------------------
	 *             Gestion des op‚rations ensemblistes 
	 -------------------------------------------------------------------*/
